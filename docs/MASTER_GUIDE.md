...
graph TB
    ...
    BUS[[Apache Kafka :9092<br/>6 topics · event bus]]
...
sequenceDiagram
    ...
    C->>O: POST /api/orders
    O->>O: save order (PENDING), compute total
    O->>K: publish order.created {orderId, userId, amount, items}

    K->>I: order.created
    I->>I: acquire Redis lock inventory:lock:{pid}
    I->>I: check available = quantity - reserved
    alt stock available
        I->>I: reserved += qty
        I->>K: inventory.updated {orderId, success:true}
    else insufficient
        I->>K: inventory.updated {orderId, success:false, reason}
    end
    I->>I: release Redis lock

    K->>P: inventory.updated (success:true only)
    P->>P: process payment, save SUCCESS
    P->>K: payment.processed {orderId, success:true, txnId}

    K->>O: payment.processed → markPaid(orderId) → status PAID
    K->>O: inventory.updated(success:false) → cancelDueToOutOfStock → CANCELLED
